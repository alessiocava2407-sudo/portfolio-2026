...
const projectsData = [
  {
    ...
    category: "WEB DEV",
    accent: mono,
    gradient: "radial-gradient(ellipse at 30% 70%, rgba(255,255,255,0.05) 0%, transparent 50%)",
  },
  {
    id: "02",
  ...
  },
  {
    ...
    category: "AUDIO & CODE",
    accent: mono,
    gradient: "radial-gradient(ellipse at 50% 50%, rgba(255,255,255,0.04) 0%, transparent 45%)",
  },
  {
    id: "04",
    title: "Arrustuta",
    ...
    category: "WEB APP",
    accent: mono,
    gradient: "radial-gradient(ellipse at 70% 60%, rgba(255,255,255,0.05) 0%, transparent 50%)",
  },
  {
    id: "05",
  ...
  },
  {
    ...
    category: "GESTIONALE",
    accent: mono,
    gradient: "radial-gradient(ellipse at 60% 70%, rgba(255,255,255,0.05) 0%, transparent 50%)",
  },
  {
    id: "07",
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
const glass = "border border-white/10 bg-white/[0.03] backdrop-blur-sm rounded-lg";
const glassSm = "border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm rounded-md";
        ...
        <div className="h-full w-full flex flex-col p-5 gap-3">
          <div className={`${glassSm} px-4 py-2.5 flex items-center justify-between`}>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-sans uppercase tracking-[0.15em] text-white/50">New Garden</span>
            </div>
            <div className="flex gap-3">
              <span className="text-[8px] font-sans uppercase tracking-wider text-white/25">Catalogo</span>
              <span className="text-[8px] font-sans uppercase tracking-wider text-white/25">Ordini</span>
              <span className="text-[8px] font-sans uppercase tracking-wider text-white/60">Dashboard</span>
            </div>
          </div>
          <div className="flex gap-2">
            {[{ label: "Ordini oggi", val: "34" }, { label: "Fatturato", val: "€ 12.4k" }, { label: "Prodotti", val: "1,280" }].map((s) => (
              <div key={s.label} className={`${glassSm} flex-1 p-3`}>
                <span className="block text-[7px] font-sans uppercase tracking-wider text-white/25 mb-1">{s.label}</span>
                <span className="block text-[16px] font-sans font-medium text-white/80 tabular-nums">{s.val}</span>
              </div>
            ))}
          </div>
          <div className={`${glassSm} flex-1 p-4 flex flex-col`}>
            <span className="text-[8px] font-sans uppercase tracking-wider text-white/25 mb-3">Vendite settimanali</span>
            <div className="flex-1 flex items-end gap-1.5">
              {[45, 72, 58, 90, 65, 82, 48].map((h, i) => (
                <motion.div
                  key={i}
                  className="flex-1 rounded-sm"
                  style={{ background: i === 3 ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.08)" }}
                  initial={{ height: 0 }}
            ...
            <div className="flex justify-between mt-2">
              {["L", "Ma", "Me", "G", "V", "S", "D"].map((d) => (
                <span key={d} className="flex-1 text-center text-[7px] text-white/20 font-sans">{d}</span>
              ))}
            </div>
          </div>
          <div className={`${glassSm} p-3`}>
            {["Rosa Rossa x120", "Orchidea Phalaenopsis x45", "Bonsai Ficus x18"].map((item, i) => (
              <div key={i} className={`flex items-center justify-between py-1.5 ${i < 2 ? "border-b border-white/[0.04]" : ""}`}>
                <span className="text-[9px] text-white/40 font-sans">{item}</span>
    ...
    case "02":
      return (
        <div className="h-full w-full flex flex-col p-5 gap-3">
          <div className={`${glassSm} px-4 py-2.5 flex items-center justify-between`}>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-serif italic text-white/60">CAI</span>
              <span className="text-[8px] font-sans uppercase tracking-wider text-white/30">Acireale</span>
            </div>
            <div className="flex gap-2">
              {["Soci", "Escursioni", "Contatti"].map((l) => (
                <span key={l} className="text-[7px] font-sans uppercase tracking-wider text-white/25">{l}</span>
              ))}
            </div>
          </div>
          <div className={`${glassSm} flex-1 relative overflow-hidden`}>
            ...
            <div className="absolute top-4 left-4 text-[8px] font-sans uppercase tracking-wider text-white/20">Etna Nord · 2,800m</div>
            <div className="absolute top-4 right-4 text-[8px] font-sans uppercase tracking-wider text-white/40">LIVE</div>
          </div>
          <div className={`${glassSm} p-3`}>
            <span className="text-[8px] font-sans uppercase tracking-wider text-white/25 block mb-2">Prossime escursioni</span>
            {[
              ...
              <div key={i} className={`flex items-center gap-3 py-1.5 ${i < 2 ? "border-b border-white/[0.04]" : ""}`}>
                <span className="text-[9px] font-sans tabular-nums text-white/30 w-10 shrink-0">{ev.date}</span>
                <span className="text-[9px] font-sans text-white/50 flex-1">{ev.name}</span>
                <span className="text-[8px] font-sans px-1.5 py-0.5 rounded bg-white/[0.06] text-white/50">{ev.diff}</span>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <div className={`${glassSm} flex-1 p-2.5 text-center`}>
              <span className="block text-[18px] font-sans font-medium text-white/70 tabular-nums">247</span>
              <span className="block text-[7px] font-sans uppercase tracking-wider text-white/25">Soci attivi</span>
            </div>
            <div className={`${glassSm} flex-1 p-2.5 text-center`}>
              <span className="block text-[18px] font-sans font-medium text-white/50 tabular-nums">12</span>
              <span className="block text-[7px] font-sans uppercase tracking-wider text-white/25">Escursioni 2026</span>
            </div>
          </div>
        </div>
      );
        ...
        <div className="h-full w-full flex flex-col p-5 gap-3 font-mono">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] uppercase tracking-wider text-white/50">MediaPipe Hands · LIVE</span>
            </div>
            <span className="text-[8px] text-white/25 tabular-nums">30 FPS · 640×480</span>
          </div>
          <div className={`${glass} flex-1 relative overflow-hidden`}>
            ...
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 200" fill="none">
              {[
                ...
                <motion.path key={i} d={p.d} stroke="rgba(255,255,255,0.25)" strokeWidth={p.w}
            ...
            <div className="absolute top-3 left-3 text-[8px] text-white/30 leading-relaxed">
              <div>x: <span className="tabular-nums text-white/50">0.482</span></div>
              <div>y: <span className="tabular-nums text-white/50">0.315</span></div>
            </div>
            <div className="absolute top-3 right-3 text-[8px] text-white/30 text-right leading-relaxed">
              <div>Confidence</div>
              <div className="tabular-nums text-white/60">0.97</div>
            </div>
            <div className="absolute bottom-3 left-3 text-[8px] text-white/30">
              Landmarks: <span className="tabular-nums text-white/50">21</span>
            </div>
          </div>
          <div className={`${glassSm} p-3 flex gap-4`}>
            {[
              ...
              <div key={m.cc} className="flex-1">
                <span className="block text-[7px] uppercase tracking-wider text-white/35">{m.cc}</span>
                <span className="block text-[8px] text-white/25">{m.label}</span>
                <span className="block text-[14px] font-medium text-white/70 tabular-nums">{m.val}</span>
              </div>
            ))}
          </div>
          <div className="flex items-center justify-between text-[8px] text-white/20">
            <span>MIDI Ch: <span className="tabular-nums text-white/40">1</span></span>
            <span>Port: <span className="text-white/40">loopMIDI</span></span>
      ...
      return (
        <div className="h-full w-full flex flex-col p-5 gap-3 font-mono">
          <div className="text-center py-2">
            <span className="text-[13px] font-serif italic text-white/60 block">Arrustuta</span>
            <span className="text-[7px] uppercase tracking-[0.3em] text-white/20 block mt-1">Resoconto Spese · Evento</span>
            <div className="w-full h-px mt-3" style={{ backgroundImage: "repeating-linear-gradient(90deg, rgba(255,255,255,0.06) 0px, rgba(255,255,255,0.06) 4px, transparent 4px, transparent 8px)" }} />
          </div>
            ...
            {[
              ...
              <div key={i} className="flex items-baseline justify-between py-2 border-b border-white/[0.04]">
                <div className="flex items-baseline gap-2 flex-1">
                  <span className="text-[9px] text-white/45">{row.item}</span>
                  <span className="text-[8px] text-white/15">{row.qty}</span>
                </div>
                <span className="text-[10px] text-white/50 tabular-nums">{row.price}</span>
              </div>
            ))}
            <div className="my-3" style={{ height: "1px", backgroundImage: "repeating-linear-gradient(90deg, rgba(255,255,255,0.1) 0px, rgba(255,255,255,0.1) 4px, transparent 4px, transparent 8px)" }} />
            <div className="flex justify-between py-1">
              <span className="text-[8px] text-white/25 uppercase tracking-wider">Subtotale</span>
              <span className="text-[10px] text-white/40 tabular-nums">€ 187,40</span>
            </div>
            <div className="flex justify-between py-1">
              <span className="text-[8px] text-white/25 uppercase tracking-wider">÷ 8 persone</span>
              <span className="text-[10px] text-white/40 tabular-nums">€ 23,43</span>
            </div>
            <div className="mt-2 p-3 rounded-lg bg-white/[0.04] border border-white/[0.08]">
              <div className="flex justify-between items-baseline">
                <span className="text-[9px] uppercase tracking-[0.2em] text-white/50">Totale</span>
                <span className="text-[22px] font-medium tabular-nums text-white/90">€ 187,40</span>
              </div>
            </div>
          </div>
          <div className="text-center pt-2">
            <span className="text-[7px] text-white/15 uppercase tracking-wider">Generato da Arrustuta · 2025</span>
          </div>
        </div>
      );
      ...
      return (
        <div className="h-full w-full flex flex-col p-5 gap-3">
          <div className={`${glassSm} px-4 py-2 flex items-center justify-between`}>
            <span className="text-[9px] font-sans uppercase tracking-[0.2em] text-white/50">Producer Tools</span>
          ...
          <div className={`${glass} flex-1 flex flex-col items-center justify-center relative`}>
            <span className="text-[8px] font-sans uppercase tracking-[0.3em] text-white/20 mb-2">Tempo</span>
            <div className="flex items-baseline gap-1">
              <span className="text-[72px] font-sans font-light leading-none tabular-nums text-white/90">128</span>
              <span className="text-[14px] text-white/25 font-sans">.00</span>
            </div>
            <span className="text-[10px] font-sans text-white/20 mt-1 uppercase tracking-wider">BPM</span>
            <div className="mt-4 px-4 py-1.5 rounded-full bg-white/[0.05] border border-white/[0.1]">
              <span className="text-[11px] font-sans text-white/60">F# minor</span>
            </div>
          ...
          <div className={`${glassSm} p-3 flex gap-3`}>
            {[
              ...
              <div key={n.label} className="flex-1">
                <span className="block text-[7px] font-sans uppercase tracking-wider text-white/25 mb-1">{n.label}</span>
                <span className="block text-[13px] font-sans text-white/60 tabular-nums">{n.val}</span>
                {i < 2 && <div className="absolute right-0 top-0 bottom-0 w-px bg-white/[0.06]" />}
              </div>
            ))}
          </div>
          <div className={`${glassSm} p-3`}>
            <span className="block text-[7px] font-sans uppercase tracking-wider text-white/25 mb-2">Spectrum</span>
            <div className="flex items-end gap-[3px] h-12">
              {spectrumBars.map((bar, i) => (
                <motion.div key={i} className="flex-1 rounded-[1px]"
                  style={{ background: i < 8 ? "rgba(255,255,255,0.35)" : i < 16 ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.1)" }}
    ...
    case "06":
      return (
        <div className="h-full w-full flex flex-col p-5 gap-3">
          <div className={`${glassSm} px-4 py-2.5 flex items-center justify-between`}>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-sans font-medium text-white/60">Band Hub</span>
              <span className="text-[7px] px-1.5 py-0.5 rounded text-white/30 bg-white/[0.05]">v2.1</span>
            </div>
            <span className="text-[8px] font-sans uppercase tracking-wider text-white/25">Membri</span>
          </div>

          {/* Team list */}
          <div className={`${glassSm} p-3 flex-1`}>
            <span className="text-[8px] font-sans uppercase tracking-wider text-white/25 block mb-3">Team</span>
            {[
              ...
              <div key={i} className={`flex items-center gap-3 py-2.5 ${i < 4 ? "border-b border-white/[0.04]" : ""}`}>
                <div className="w-7 h-7 rounded-full bg-white/[0.06] flex items-center justify-center shrink-0">
                  <span className="text-[8px] font-sans font-medium text-white/40">{m.initials}</span>
                </div>
                <span className="text-[10px] font-sans text-white/50 flex-1">{m.name}</span>
                <span className={`text-[7px] px-2 py-0.5 rounded-full font-sans ${i === 0 ? "bg-white/[0.1] text-white/60" : "bg-white/[0.04] text-white/30"}`}>
                  {m.role}
                </span>
              </div>
            ))}
          ...
          {/* Next event */}
          <div className={`${glassSm} p-3`}>
            <span className="text-[8px] font-sans uppercase tracking-wider text-white/25 block mb-2">Prossimo evento</span>
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[10px] font-sans text-white/60 block">Live @ Monk — Catania</span>
                <span className="text-[8px] font-sans text-white/25">Sab 12 Lug · 22:00</span>
              </div>
              <span className="text-[8px] px-2 py-1 rounded-full bg-white/[0.06] text-white/40 font-sans">Confermato</span>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <div className={`${glassSm} flex-1 p-2 text-center`}>
              <span className="block text-[14px] font-sans font-medium text-white/60 tabular-nums">12</span>
              <span className="block text-[7px] font-sans uppercase tracking-wider text-white/20">Prossimi live</span>
            </div>
            <div className={`${glassSm} flex-1 p-2 text-center`}>
              <span className="block text-[14px] font-sans font-medium text-white/50 tabular-nums">€ 2,340</span>
              <span className="block text-[7px] font-sans uppercase tracking-wider text-white/20">Cachet totale</span>
            </div>
          </div>
        </div>
      );
    ...
    case "07":
      return (
        <div className="h-full w-full flex flex-col p-5 gap-3">
          ...
          <div className="grid grid-cols-2 gap-2 flex-1">
            {[
              ...
              <div key={i} className={`${glassSm} relative overflow-hidden flex items-end p-3 ${item.h}`} style={{ minHeight: i === 0 ? undefined : "70px" }}>
                ...
                <span className="text-[9px] font-sans text-white/40 relative z-10">{item.label}</span>
              </div>
            ))}
            <div className={`${glassSm} col-span-2 p-3 flex items-center justify-between`}>
              <span className="text-[9px] font-sans text-white/40">Materiali e Tessuti</span>
              <div className="flex gap-1.5">
                {["rgba(255,255,255,0.15)", "rgba(255,255,255,0.08)", "rgba(255,255,255,0.25)", "rgba(255,255,255,0.04)"].map((c, i) => (
          ...
          <div className={`${glassSm} px-4 py-2.5 flex items-center justify-between`}>
            <span className="text-[8px] font-sans text-white/30">Via Fabio, 16 · Acireale</span>
            <span className="text-[8px] font-sans px-2 py-1 rounded-full bg-white/[0.06] text-white/50">Preventivo</span>
          </div>
        </div>
      );
    ...
    case "08":
      return (
        <div className="h-full w-full flex flex-col p-5 gap-3">
          <div className={`${glassSm} px-4 py-2.5 flex items-center justify-between`}>
            <div>
              <span className="text-[12px] font-serif italic text-white/60">Dimora Akis</span>
              <span className="text-[7px] font-sans text-white/20 block">B&amp;B · Acireale</span>
            </div>
            <div className="flex gap-2">
              <span className="text-[7px] font-sans uppercase tracking-wider text-white/25">Camere</span>
              <span className="text-[7px] font-sans uppercase tracking-wider text-white/50">Prenota</span>
            </div>
          </div>
          <div className={`${glass} p-4`}>
            <span className="text-[8px] font-sans uppercase tracking-wider text-white/25 block mb-3">Seleziona date</span>
            <div className="flex gap-3 mb-3">
              <div className="flex-1 p-2 rounded-md border border-white/[0.06] bg-white/[0.02]">
                <span className="block text-[7px] font-sans text-white/20 uppercase">Check-in</span>
                <span className="block text-[12px] font-sans text-white/60 tabular-nums">15 Lug 2026</span>
              </div>
              <div className="flex items-center text-white/15 text-[12px]">→</div>
              <div className="flex-1 p-2 rounded-md border border-white/[0.06] bg-white/[0.02]">
                <span className="block text-[7px] font-sans text-white/20 uppercase">Check-out</span>
                <span className="block text-[12px] font-sans text-white/60 tabular-nums">18 Lug 2026</span>
              </div>
            </div>
            <div className="grid grid-cols-7 gap-1 text-center">
              {["L", "Ma", "Me", "G", "V", "S", "D"].map((d) => (
                <span key={d} className="text-[7px] text-white/15 font-sans py-0.5">{d}</span>
              ))}
              {Array.from({ length: 31 }, (_, i) => {
                ...
                return (
                  <span key={day}
                    className={`text-[8px] font-sans tabular-nums py-1 ${isSel ? "text-white/90" : day < 15 ? "text-white/15" : "text-white/35"}`}
                    style={isSel ? {
                      background: isEdge ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.06)",
          ...
          <div className={`${glassSm} p-3 flex items-center gap-3`}>
            <div className="w-14 h-14 rounded-lg shrink-0 flex items-center justify-center bg-white/[0.04]">
              <span className="text-[11px] font-serif italic text-white/30">Aci</span>
            </div>
            <div className="flex-1">
              <span className="text-[10px] font-sans font-medium text-white/60">Camera Aci</span>
              <span className="block text-[8px] font-sans text-white/25">2 ospiti · Vista mare · Colazione</span>
              <div className="flex items-baseline gap-1 mt-1">
                <span className="text-[14px] font-sans font-medium tabular-nums text-white/70">€ 89</span>
                <span className="text-[8px] text-white/25">/ notte</span>
              </div>
            </div>
            <div className="text-[8px] px-2.5 py-1.5 rounded-full font-sans bg-white/[0.06] text-white/50">Disponibile</div>
          </div>
          <div className="p-3 rounded-xl text-center bg-white/[0.06] border border-white/[0.1]">
            <span className="text-[11px] font-sans font-medium text-white/70">Prenota · 3 notti · € 267</span>
          </div>
        </div>
      );
    ...
    default:
      return (
        <div className="h-full w-full flex items-center justify-center">
          <span className="font-serif italic text-[8rem] text-white/[0.04] select-none">{id}</span>
        </div>
      );
  }
}
...
function ProjectCard({ project, index }) {
  ...
  return (
    <motion.article
      ...
      style={{
        "--card-glow-shadow": "0 0 80px -20px rgba(255,255,255,0.06)",
    ...
    >
      <div className={`${isMobile ? "w-full" : "w-1/2"} shrink-0 relative overflow-hidden`} style={{ background: project.gradient, minHeight: isMobile ? "280px" : "420px" }}>
        <motion.div className="absolute inset-0 pointer-events-none z-0"
          style={{ background: "radial-gradient(circle at 50% 50%, rgba(255,255,255,0.04) 0%, transparent 60%)" }}
          animate={{ opacity: hovered ? 1 : 0 }} transition={{ duration: 0.7 }} />
        <div className="relative z-10 h-full">
          {renderProjectVisual(project.id, project.accent)}
        </div>
      </div>
      <div className={`${isMobile ? "w-full" : "w-1/2"} flex flex-col justify-center p-6 md:p-10 lg:p-14 bg-white/[0.01]`}>
        <span className="block text-[9px] md:text-[10px] font-sans uppercase tracking-[0.3em] mb-2 tabular-nums text-white/30">
          {project.id} / 08
        </span>
    ...
    <>
      ...
      <header className="fixed top-0 inset-x-0 z-50 mix-blend-difference pointer-events-none">
        <div className="flex items-center justify-between px-6 md:px-12 py-5">
          ...
          <nav className="hidden md:flex items-center gap-1 text-[11px] font-sans uppercase tracking-[0.2em] text-white/50 pointer-events-auto">
            <a href="#studio" className="px-3 py-1 hover:text-white transition-colors duration-500">Studio</a>
            <span className="text-white/15">·</span>
            <a href="#lavori" className="px-3 py-1 hover:text-white transition-colors duration-500">Lavori</a>
            <span className="text-white/15">·</span>
            <a href="#competenze" className="px-3 py-1 hover:text-white transition-colors duration-500">Competenze</a>
            <span className="text-white/15 mx-2">|</span>
            <span className="text-white/25 tabular-nums">Selected Work · 2020 — 2026</span>
          </nav>
        </div>
      </header>
      ...
      <main>
        ...
        <section id="top" className="min-h-screen flex flex-col justify-end px-4 sm:px-6 md:px-12 lg:px-24 pb-12 sm:pb-16 md:pb-24 pt-24 sm:pt-32 relative overflow-hidden">
          ...
          <motion.svg
            ...
            style={{ y: layerBgY }}
          >
            <motion.circle cx="100" cy="100" r="90" fill="none" stroke="rgba(255,255,255,0.025)" strokeWidth="0.5"
              initial={{ pathLength: 0, rotate: -90 }} animate={{ pathLength: 0.75 }}
              transition={{ duration: 3, ease: easeApple, delay: 1.2 }}
              style={{ transformOrigin: "center" }} />
            <motion.circle cx="100" cy="100" r="60" fill="none" stroke="rgba(255,255,255,0.015)" strokeWidth="0.5"
          ...
          <motion.div className="max-w-[1400px] mx-auto w-full relative z-10" style={{ opacity: heroOpacity }}>
            ...
            <motion.div style={{ y: layer1Y }}>
              ...
              >
                <span className="block text-[9px] sm:text-[10px] md:text-[11px] font-sans uppercase tracking-[0.35em] text-white/25 mb-6 sm:mb-8 md:mb-12">
            ...
            <motion.div style={{ y: layer2Y, scale: heroScale }}>
              ...
              >
                ...
                <motion.h1
                  className="font-serif italic text-[clamp(3rem,10vw,8rem)] leading-[0.9] tracking-[-0.04em] text-white/70 overflow-hidden"
                  style={{ y: heroNameY, scale: heroPulse, textShadow: heroNameShadow }}
                >
            ...
            <motion.div
              ...
              style={{ background: "linear-gradient(90deg, rgba(255,255,255,0.15), transparent)", transformOrigin: "left" }}
            ...
            <motion.div style={{ y: layer3Y }}>
              <motion.p
                ...
                className="font-sans text-[15px] md:text-[20px] leading-[1.7] text-white/45 max-w-full sm:max-w-[600px] font-light"
              >
            ...
            >
              ...
              <motion.div
                className="hidden md:flex items-center gap-2 text-[10px] font-sans uppercase tracking-[0.2em] text-white/20"
                initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        ...
        <div className="border-y border-white/[0.04] overflow-hidden py-7">
          <motion.div className="flex gap-10 whitespace-nowrap"
            ...
            {[0, 1].map((copy) => (
              <span key={copy} className="flex items-center gap-10 text-[12px] font-sans uppercase tracking-[0.3em] text-white/20">
                {marqueeItems.map((item, idx) => (
                  <span key={`${copy}-${idx}`} className="flex items-center gap-10">
                    {item.style === "serif" ? (
                      <span className="font-serif italic normal-case text-white/40 text-[16px] tracking-normal">{item.text}</span>
                    ) : (
                      <span>{item.text}</span>
                    )}
        ...
        <section id="studio" className="px-6 md:px-12 py-32 md:py-48">
          <div className="max-w-[1400px] mx-auto">
            <motion.div {...reveal}>
              <span className="block text-[11px] font-sans uppercase tracking-[0.3em] text-white/20 mb-6">01 — Studio</span>
            </motion.div>
            <motion.div {...revealDeep}>
              <motion.h2 variants={revealTextStrong} initial="initial" whileInView="whileInView" viewport={revealTextStrong.viewport} className="font-serif text-[clamp(2.8rem,7vw,6rem)] leading-[1.05] tracking-[-0.025em] mb-20 md:mb-28">
                Lo <span className="italic text-white/60">studio</span> &amp; il metodo.
              </motion.h2>
            </motion.div>
            <div className="grid md:grid-cols-12 gap-12 md:gap-24">
              <motion.div className="md:col-span-4" {...reveal}>
                <div className="aspect-[4/5] rounded-2xl border border-white/[0.05] bg-white/[0.02] backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="md:col-span-7 md:col-start-6 space-y-16">
                {[
                  { num: "i.", text: <>Sono studente di <strong className="text-white/90 font-semibold">Ingegneria Informatica all&apos;Università di Catania</strong>. Quello che so fare davvero, però, l&apos;ho imparato costruendo: un approccio da autodidatta, fatto di tentativi e iterazione lucida.</> },
                  { num: "ii.", text: <>Da una parte progetto soluzioni web — front-end, back-end, gestionali — pensate per <strong className="text-white/90 font-semibold">risolvere problemi reali</strong>. Dall&apos;altra produco musica, suono più strumenti e studio l&apos;hardware audio dal suo lato circuitale.</> },
                  { num: "iii.", text: <>Il filo conduttore è uno solo: <em className="font-serif italic text-white/70">capire come funzionano le cose dentro</em>. Un&apos;API, un compressore, una catena di segnale. Tutto è un sistema che si può smontare, capire e rimettere insieme meglio.</> },
                ].map((p, i) => (
                  <motion.div key={p.num}
                    ...
                    <span className="block text-[11px] font-sans uppercase tracking-[0.3em] text-white/15 mb-3">{p.num}</span>
                    <p className="text-[18px] md:text-[22px] leading-[1.55] text-white/40 font-light">{p.text}</p>
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <div className={`${isMobile ? "relative" : "sticky top-0"} flex ${isMobile ? "flex-col" : "h-screen"} flex-col justify-center overflow-hidden`}>
            <div className="px-4 sm:px-6 md:px-12 pt-12 md:pt-20 pb-8 shrink-0">
              <div className="max-w-[1400px] mx-auto flex items-end justify-between">
                <div>
                  <motion.span className="block text-[10px] sm:text-[11px] font-sans uppercase tracking-[0.3em] text-white/20 mb-3 md:mb-4"
              ...
              >02 — Lavori selezionati</motion.span>
                  <motion.h2 className="font-serif text-[2rem] sm:text-[2.5rem] md:text-[clamp(2rem,5vw,4rem)] leading-[1.05] tracking-[-0.025em]" {...revealDeep}>
                    Otto progetti, <span className="italic text-white/60">due discipline</span>.
                  </motion.h2>
                </div>
                <span className="hidden md:block text-[10px] font-sans uppercase tracking-[0.28em] text-white/20 tabular-nums">01 — 08</span>
              </div>
            </div>
        ...
        <section id="competenze" className="px-4 sm:px-6 md:px-12 lg:px-24 py-16 sm:py-24 md:py-32 lg:py-48">
          <div className="max-w-[1400px] mx-auto">
            <motion.div {...revealText}>
              <span className="block text-[9px] sm:text-[10px] md:text-[11px] font-sans uppercase tracking-[0.3em] text-white/20 mb-6">03 — Competenze</span>
            </motion.div>
            <motion.div {...revealDeep} className="mb-20">
              <motion.h2 variants={revealTextStrong} initial="initial" whileInView="whileInView" viewport={revealTextStrong.viewport} className="font-serif text-[2rem] sm:text-[2.8rem] md:text-[clamp(2.8rem,7vw,6rem)] leading-[1.05] tracking-[-0.025em]">
                Strumenti che uso <span className="italic text-white/60">tutti i giorni</span>.
              </motion.h2>
            </motion.div>
                ...
                {SKILLS_WEB.map(([name, detail], i) => (
                  <motion.div key={name}
                    ...
                    <div className="group flex items-baseline justify-between py-5 border-b border-white/[0.04] text-[14px] sm:text-[15px] md:text-[17px] hover:border-white/[0.1] transition-colors duration-500">
                      <span className="text-white/75 font-medium group-hover:text-white transition-colors duration-500">{name}</span>
                      <span className="text-[11px] sm:text-[12px] text-white/20 font-sans group-hover:text-white/40 transition-colors duration-500">{detail}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
        ...
        <footer className="px-4 sm:px-6 md:px-12 lg:px-24 pt-16 sm:pt-24 md:pt-32 lg:pt-48 pb-12 sm:pb-16 md:pb-20">
          <div className="max-w-[1400px] mx-auto">
            <motion.div {...reveal}>
              <span className="block text-[9px] sm:text-[10px] md:text-[11px] font-sans uppercase tracking-[0.3em] text-white/20 mb-6">04 — Contatti</span>
            </motion.div>
            <motion.div {...revealDeep}>
              <motion.h2 variants={revealTextStrong} initial="initial" whileInView="whileInView" viewport={revealTextStrong.viewport} className="font-serif text-[1.5rem] sm:text-[1.875rem] md:text-[clamp(2rem,6vw,4rem)] leading-[1.05] tracking-[-0.025em] mb-10">
                Hai un&apos;idea che <span className="italic text-white/60">vuole prendere forma?</span>
              </motion.h2>
            </motion.div>
            <motion.div {...reveal}>
              <motion.p variants={revealText} initial="initial" whileInView="whileInView" viewport={revealText.viewport} className="text-white/30 text-[13px] sm:text-[14px] md:text-[15px] lg:text-[17px] max-w-full sm:max-w-[520px] leading-[1.8] font-light mb-14">
                Codice, suono, hardware — o tutto insieme. Scrivimi due righe, troviamo il modo migliore di partire.
              </motion.p>
            </motion.div>
            <motion.div {...reveal}>
              <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                <motion.a href="mailto:alessio.cavallaro@email.com"
            ...
            <motion.div {...reveal}>
              <div className="flex flex-col md:flex-row justify-between gap-4 text-[11px] font-sans text-white/15">
                <div className="flex items-center gap-3">
                  <span className="font-serif italic text-[17px] text-white/35">Alessio Cavallaro</span>
                  <span className="hidden md:inline">·</span>
                  <span className="uppercase tracking-[0.2em]">Catania, IT · 2026</span>
                </div>
                <a href="#top" className="uppercase tracking-[0.2em] hover:text-white/40 transition-colors duration-500">Torna su ↑</a>
              </div>
            </motion.div>
          </div>
        </footer>
